...
test.concurrent.for(testCases)(
	'@svelbase/svelbase $kind.type $variant',
	async (testCase, { page, ...ctx }) => {
		const cwd = ctx.cwd(testCase);

		const msg = "Community Addon Template demo for the add-on: '@svelbase/svelbase'!";

		const contentPath = path.resolve(cwd, `src/lib/@svelbase/svelbase/content.txt`);
		const contentContent = fs.readFileSync(contentPath, 'utf8');
		// Check if we have the imports
		expect(contentContent).toContain(msg);

		const helloPath = path.resolve(cwd, `src/lib/@svelbase/svelbase/HelloComponent.svelte`);
